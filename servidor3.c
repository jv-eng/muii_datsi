...
static void promocionar_cap() {
    ...
    cap_set_flag(capabilities, CAP_PERMITTED, 1, cap_list, CAP_SET);
...
static void descartar_cap(int temp) {
    ...
    cap_value_t cap_list[1] = {CAP_NET_BIND_SERVICE};
    if (temp == 1) {
        cap_set_flag(capabilities, CAP_EFFECTIVE, 1, cap_list, CAP_CLEAR);
        cap_set_flag(capabilities, CAP_PERMITTED, 1, cap_list, CAP_CLEAR);
    } else {
        cap_set_flag(capabilities, CAP_EFFECTIVE, 1, cap_list, CAP_SET);
        cap_set_flag(capabilities, CAP_PERMITTED, 1, cap_list, CAP_SET);
    }
