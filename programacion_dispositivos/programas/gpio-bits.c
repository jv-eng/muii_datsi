...
int main(int argc, char *argv[]) {
	...
	gpio[off_selec+pin/10] = bits_inserta(gpio[off_selec+pin/10], (off_selec+pin%10)*3, 3, 1);
	
	gpio[off_set+pin/32] = bit_fija_a_uno(0, pin%32); // activa el pin
	
	gpio[off_clear+pin/32] = bit_fija_a_uno(0, pin%32); // desactiva el pin
