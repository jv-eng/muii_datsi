...
static uint8_t leer_reg(uint8_t reg){
	outb(reg, RTC_REG); return inb(RTC_DAT); }
static int a_binario(int binario, uint8_t n) { // de BCD a binario si necesario
	return (binario ? n : (n>>4) * 10) + (n & 0xF); }
static int rtc_init(void) {
	int pm=0;
	uint8_t leido;
        uint8_t formato = leer_reg(RTC_STATB); // accede a reg. estado B
	int binario = formato & 0x04; // binario o BCD
	int formato24h = formato & 0x02; // 24h o 12h
	...
	leido = leer_reg(RTC_HORA); 
	if (!formato24h && (leido & 0x80)){ // hora PM si formato 12h y bit de mayor peso activo
		pm=1; leido &= 0x07; } // elimino bit de mayor peso 
